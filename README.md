...
- **別インストールの保護**: 対象プレフィックス（例 `wp_`）で始まっても、別のWordPressインストール（例 `wp_old_`）に属すると判断したテーブルは削除せず保護します（確認画面に一覧表示）。マルチサイトのサブサイトテーブル（`wp_2_` 等）は同一インストールの一部として削除対象に含めます。
- **ドキュメントルート直下の保護**: WordPressがドキュメントルート直下にある場合、`.htaccess` / `.htpasswd` / `.user.ini` はWordPress以外の設定を含む可能性があるため削除しません。
